...
#
#     text_to_summarize = text_to_summarize[:2000]
#     prompt_length = len(text_to_summarize)
#     max_toks = estimate_max_tokens(128)
#     local_llm = get_llm(max_toks)
#
#     summarization_type = state["summarization_type"]
#     if summarization_type == "extractive":
#         prompt = f"""ВЫВОДИ ТОЛЬКО КЛЮЧЕВЫЕ ПРЕДЛОЖЕНИЯ. НИЧЕГО БОЛЬШЕ. НИКАКИХ ПОЯСНЕНИЙ.
#         print(f"Суммаризация: max_tokens={max_toks}, длина текста={len(text_to_summarize)}")
#
# Текст:
# {text_to_summarize}
#
# Ответ:"""
#     else:
#         prompt = f"""СОЗДАЙ КРАТКОЕ СОДЕРЖАНИЕ В 1 ПРЕДЛОЖЕНИИ. ТОЛЬКО ОТВЕТ. НИЧЕГО БОЛЬШЕ
#         print(f"Суммаризация: max_tokens={max_toks}, длина текста={len(text_to_summarize)}")
#
# Текст:
# {text_to_summarize}
...
#
#     try:
#         raw_summary = local_llm.invoke(prompt)  # ← local_llm
...
#     """
#     Извлекает ПЕРВЫЙ валидный JSON из текста (а не последний).
#     Это важно, когда модель сначала даёт JSON, а потом рассуждает.
#     """
...
#         max_toks = estimate_max_tokens(prompt_length)
#         local_llm = get_llm(max_toks)
#
